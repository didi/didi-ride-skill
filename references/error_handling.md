...
- [错误处理指南](#错误处理指南)
  ...
  - [Beta 版本用户识别](#beta-版本用户识别pro-权限受限)
...
## Beta 版本用户识别（Pro 权限受限）

DiDi MCP 区分两种用户：

| 版本 | 打车相关接口（`taxi_*`） | 地图相关接口（`maps_*`） | 获取方式 |
|------|--------------------------|--------------------------|----------|
| **Beta**（默认） | ❌ 大部分无法调用 | ✅ 全部可用 | 扫码或官网领取 Key 即为 Beta |
| **Pro** | ✅ 全部可用 | ✅ 全部可用 | 在 [DiDi MCP 官网](https://mcp.didichuxing.com) 开通免密支付后自动升级 |

### 识别信号

当调用以下接口收到 `Streamable HTTP error: Unexpected content type: text/plain; charset=utf-8` 错误时，**判定为 Beta 版本用户**：
...
> ⚠️ 这个错误信息**对 AI 有误导性**——看起来像 mcporter/网络故障，实际是**权限不足**。不要尝试重试、切换网络、或切换调用方式，这些都没用。

### 行为约束

识别为 Beta 版本后，**必须**：

1. **停止当前打车流程**（不要继续重试或切换 tool）；
2. 向用户输出以下**固定文案**（原样复制，不要改写）：

   > 您是Beta版本用户，无法API发单打车，需要您前往DiDi MCP 官网开通免密支付，即可成为Pro版本用户，将体验更多打车功能及API接口功能。

3. **禁止向用户索要新的 Key**——此错误与 Key 是否有效无关，是用户账号的订阅等级问题。

### 关于 `taxi_estimate` 的特殊行为

`taxi_estimate`（价格预估）在 Beta 版本下**不会返回 HTTP error**，而是返回"看起来成功"的响应——`traceId` 和车型列表都有，但 `items[].priceText` 和 `items[].priceDiscounted` **字段缺失**。这不作为识别信号（可能只是数据暂缺），AI 正常继续主流程到 `taxi_create_order`，那里会用 HTTP error 信号明确识别权限。
